...
import sys
sys.path.append("..")
from mywork import tulingRobot,faceRecog,vadSound,BaiduSDK,light,yuntai,motivation,ps2
...
def main():
    ...
    print('按键启动')
    while True:
        ps=ps2.PS2KEY(35,33,31,29)
        if ps.ps2_key():
            print(ps.ps2_key())
            if ps.ps2_key() == 16:
                yuntai.setcarAngle_back()  #复原舵机初始位置  【粉色方格键】
            if ps.ps2_key() == 5:  #控制小车前进   【左上】
                smartcar.forward()
            if ps.ps2_key() == 7:  #控制小车后退   【左下】
                smartcar.reverse()
            if ps.ps2_key() == 15:  #控制小车停止 【蓝色十字键】
                smartcar.brake()

            if ps.ps2_key() == 8: #控制小车右转   【左右】
                yuntai.turn_right()
            
            if ps.ps2_key() == 6: #控制小车左转   【左左】
                yuntai.turn_left()

            if ps.ps2_key() == 11: #控制小车直走   【L1】
                yuntai.straight()
...
def call_back_chat():  #自定义回调函数
    ...
    while True:
        ...
        if text_inputs == '关机':
            vadSound.play_sound('guanji.wav')
            c_light.light_off()
            break
        ...
        if text_inputs == '开启跟随' or text_inputs == '开始跟随':
            while True:
        ...
        if text_inputs == '远程控制':
            ...
            if BaiduSDK.text2sound(tuling_text):
                ...
                vadSound.play_sound('fail_TTS.wav')
                print('语音合成出错。。。')
...
def wake_up():
    ...
    detector = snowboydecoder.HotwordDetector('resources/xiaofo.pmdl', sensitivity=0.5)
...
#python snowboy.py xiaofo.pmdl 运行语句
